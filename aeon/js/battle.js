// ============================================================
// AEON :: BATTLE SYSTEM
// Stat-based resolution + real-time visualization that plays
// out the predetermined outcome.
// ============================================================

function computeBattlePower(civ, defender = false) {
  let power = civ.army * civ.weaponTechLevel;

  // Focus modifiers
  if (civ.focus === 'military') power *= 1.30;
  if (civ.focus === 'magic') power *= (1 + Math.min(0.5, civ.magic / 500));
  if (civ.focus === 'faith') power *= (1 + civ.morale / 300);

  // Government
  if (civ.government === 'theocracy' && civ.focus === 'faith') power *= 1.15;

  // Morale
  power *= (0.5 + civ.morale / 200);

  // Stability
  power *= (0.7 + civ.stability / 333);

  // Defender bonus
  if (defender) {
    power *= 1.2;
    if (civ.biome === 'mountain') power *= 1.25;
    if (civ.biome === 'tundra' || civ.biome === 'swamp') power *= 1.15;
    power *= getMod(civ, 'defense');
  }

  // Special weapon
  if (civ.weaponUnlocked && civ.weapon) {
    const w = WEAPONS[civ.weapon];
    if (w && w.battleMod) {
      const bm = w.battleMod;
      let weaponMult = 1.0;
      if (bm.ranged)       weaponMult *= bm.ranged;
      if (bm.shock)        weaponMult *= bm.shock;
      if (bm.attack)       weaponMult *= bm.attack;
      if (bm.elite)        weaponMult *= bm.elite;
      if (bm.antiDefense)  weaponMult *= bm.antiDefense * 0.6;
      if (bm.preBattle)    weaponMult *= (1 + (1 - bm.preBattle));
      if (bm.raise)        weaponMult *= 1.3;
      power *= Math.max(1.2, weaponMult * 0.6 + 0.4); // dampen
    }
  }

  return Math.floor(power);
}

// Resolve the final battle. Returns full outcome.
function resolveBattle(civA, civB, world, rng) {
  // Who is the invader? Whoever has more momentum / weaker home advantage
  // Simple heuristic: more aggressive focus invades.
  const aggressionScore = {
    military: 3, magic: 1, faith: 2, industry: 1,
    naturalism: -1, trade: -2, science: 0,
  };
  const aAgg = (aggressionScore[civA.focus] || 0) + (civA.government === 'tribal' ? 2 : 0);
  const bAgg = (aggressionScore[civB.focus] || 0) + (civB.government === 'tribal' ? 2 : 0);

  let invader, defender;
  if (aAgg > bAgg) { invader = civA; defender = civB; }
  else if (bAgg > aAgg) { invader = civB; defender = civA; }
  else { invader = rng.chance(0.5) ? civA : civB; defender = (invader === civA) ? civB : civA; }

  const invPower = computeBattlePower(invader, false);
  const defPower = computeBattlePower(defender, true);

  // RNG band ±10%
  const invRoll = invPower * rng.range(0.90, 1.10);
  const defRoll = defPower * rng.range(0.90, 1.10);

  const winner = invRoll > defRoll ? invader : defender;
  const loser  = (winner === civA) ? civB : civA;

  // Casualty calculation
  const ratio = Math.min(invRoll, defRoll) / Math.max(invRoll, defRoll);
  const winnerCasualtyPct = 0.20 + ratio * 0.40;
  const loserCasualtyPct  = 0.60 + (1 - ratio) * 0.35;

  return {
    invader, defender, winner, loser,
    invPower, defPower, invRoll, defRoll,
    winnerCasualtyPct, loserCasualtyPct,
    ratio,
  };
}

// ============================================================
// BATTLE VISUALIZATION
// Spawns unit sprites that march toward the contested middle,
// then animates a clash that reflects the resolved outcome.
// ============================================================
class BattleVisualizer {
  constructor(renderer, map, civA, civB, outcome) {
    this.renderer = renderer;
    this.map = map;
    this.civA = civA;
    this.civB = civB;
    this.outcome = outcome;
    this.units = [];
    this.phase = 'spawn';     // spawn -> march -> clash -> resolve -> done
    this.phaseTime = 0;
    this.maxUnitsPerSide = 200;
    this.frames = 0;
    this.battleLog = [];
    this.specialFired = { A: false, B: false };
  }

  start() {
    // Spawn units near each capital
    const spawnA = this.map.capitalA;
    const spawnB = this.map.capitalB;

    const aCount = Math.min(this.maxUnitsPerSide, Math.max(20, Math.floor(this.civA.army / 100)));
    const bCount = Math.min(this.maxUnitsPerSide, Math.max(20, Math.floor(this.civB.army / 100)));

    for (let i = 0; i < aCount; i++) {
      this.units.push(this.createUnit('A', spawnA));
    }
    for (let i = 0; i < bCount; i++) {
      this.units.push(this.createUnit('B', spawnB));
    }

    this.renderer.battleUnits = this.units;
    this.phase = 'march';
    this.battleLog.push({ year: 1000, text: `${this.civA.name} fields ${formatNum(this.civA.army)} soldiers.` });
    this.battleLog.push({ year: 1000, text: `${this.civB.name} fields ${formatNum(this.civB.army)} soldiers.` });
    this.battleLog.push({ year: 1000, text: `${this.outcome.invader.name} marches to attack.` });
  }

  createUnit(side, near) {
    const civ = side === 'A' ? this.civA : this.civB;
    return {
      side,
      x: near.x + (Math.random() - 0.5) * 8,
      y: near.y + (Math.random() - 0.5) * 8,
      tx: 0, ty: 0,
      color: civ.color,
      hp: 1.0,
      dead: false,
      target: null,
      elite: civ.weapon === 'champions' && Math.random() < 0.05,
      speed: 0.12 + Math.random() * 0.08,
    };
  }

  step() {
    this.frames++;
    this.phaseTime++;

    if (this.phase === 'march') {
      this.stepMarch();
    } else if (this.phase === 'clash') {
      this.stepClash();
    } else if (this.phase === 'resolve') {
      this.stepResolve();
    }

    this.renderer.updateParticles();
  }

  stepMarch() {
    const midX = this.map.width / 2;
    const midY = this.map.height / 2;

    // Special weapon trigger early in march
    if (this.phaseTime === 30 && this.civA.weaponUnlocked && !this.specialFired.A) {
      this.fireSpecial(this.civA);
      this.specialFired.A = true;
    }
    if (this.phaseTime === 45 && this.civB.weaponUnlocked && !this.specialFired.B) {
      this.fireSpecial(this.civB);
      this.specialFired.B = true;
    }

    let movingCount = 0;
    for (const u of this.units) {
      if (u.dead) continue;
      // March toward center, with vertical spread
      const targetX = midX + (u.side === 'A' ? 1 : -1) * (-2);
      const targetY = midY + (Math.sin((u.x + u.y) * 0.3) * 6);
      const dx = targetX - u.x;
      const dy = targetY - u.y;
      const d = Math.sqrt(dx*dx + dy*dy);
      if (d > 1) {
        u.x += (dx / d) * u.speed;
        u.y += (dy / d) * u.speed;
        movingCount++;
      }
    }

    if (this.phaseTime > 120 || movingCount < this.units.length * 0.3) {
      this.phase = 'clash';
      this.phaseTime = 0;
      this.battleLog.push({ year: 1000, text: 'The armies clash!' });
    }
  }

  stepClash() {
    // Determine winner influence: stronger side kills faster
    const outcome = this.outcome;
    const winnerSide = outcome.winner.side;
    const loserSide = outcome.loser.side;

    // Pair units up and fight
    const alive = this.units.filter(u => !u.dead);
    const aliveA = alive.filter(u => u.side === 'A');
    const aliveB = alive.filter(u => u.side === 'B');

    // Each frame: small percentage take casualties
    const winnerLossRate = 0.005 + outcome.winnerCasualtyPct * 0.0002;
    const loserLossRate  = 0.012 + outcome.loserCasualtyPct  * 0.0003;

    const winnerAlive = winnerSide === 'A' ? aliveA : aliveB;
    const loserAlive  = loserSide  === 'A' ? aliveA : aliveB;

    const winnerKills = Math.ceil(winnerAlive.length * winnerLossRate);
    const loserKills  = Math.ceil(loserAlive.length  * loserLossRate);

    // Kill units
    for (let i = 0; i < winnerKills; i++) {
      const u = winnerAlive[Math.floor(Math.random() * winnerAlive.length)];
      if (u && !u.dead) {
        u.dead = true;
        this.renderer.addParticle(u.x, u.y, '#aa3030', 25);
      }
    }
    for (let i = 0; i < loserKills; i++) {
      const u = loserAlive[Math.floor(Math.random() * loserAlive.length)];
      if (u && !u.dead) {
        u.dead = true;
        this.renderer.addParticle(u.x, u.y, '#aa3030', 25);
      }
    }

    // Small swirl movement
    for (const u of this.units) {
      if (u.dead) continue;
      u.x += (Math.random() - 0.5) * 0.15;
      u.y += (Math.random() - 0.5) * 0.15;
    }

    // End clash when loser is mostly gone or after time limit
    const loserCountInitial = (loserSide === 'A' ? this.units.filter(u=>u.side==='A').length : this.units.filter(u=>u.side==='B').length);
    if (loserAlive.length < loserCountInitial * 0.15 || this.phaseTime > 240) {
      this.phase = 'resolve';
      this.phaseTime = 0;
      this.battleLog.push({ year: 1000, text: `${outcome.winner.name} routs the enemy!` });
    }
  }

  stepResolve() {
    // Loser units flee/die; winner advances and takes territory
    const outcome = this.outcome;
    const loserSide = outcome.loser.side;

    // Kill remaining losers
    for (const u of this.units) {
      if (u.dead) continue;
      if (u.side === loserSide && Math.random() < 0.05) {
        u.dead = true;
        this.renderer.addParticle(u.x, u.y, '#aa3030', 20);
      }
    }

    // Winner advances and paints territory
    const winnerSide = outcome.winner.side;
    for (const u of this.units) {
      if (u.dead || u.side !== winnerSide) continue;
      // Move outward
      const dirX = winnerSide === 'A' ? 1 : -1;
      u.x += dirX * 0.25;
      u.y += (Math.random() - 0.5) * 0.1;
      // Paint
      const tx = Math.floor(u.x);
      const ty = Math.floor(u.y);
      if (tx >= 0 && tx < this.map.width && ty >= 0 && ty < this.map.height) {
        this.map.tiles[ty][tx].owner = winnerSide;
      }
    }

    if (this.phaseTime > 200) {
      // Sweep the rest of the map to the winner
      for (let y = 0; y < this.map.height; y++) {
        for (let x = 0; x < this.map.width; x++) {
          this.map.tiles[y][x].owner = winnerSide;
        }
      }
      this.phase = 'done';
    }
  }

  fireSpecial(civ) {
    const w = WEAPONS[civ.weapon];
    if (!w) return;
    this.battleLog.push({ year: 1000, text: `${civ.name} unleashes ${w.name}!` });

    // Visual flair: burst of particles at enemy positions
    const enemySide = civ.side === 'A' ? 'B' : 'A';
    const enemies = this.units.filter(u => u.side === enemySide && !u.dead);
    const target = enemies[Math.floor(Math.random() * enemies.length)];
    if (target) {
      for (let i = 0; i < 40; i++) {
        this.renderer.addParticle(
          target.x + (Math.random() - 0.5) * 6,
          target.y + (Math.random() - 0.5) * 6,
          civ.color, 40
        );
      }
      // Special weapons cause immediate casualties
      const killCount = Math.floor(enemies.length * 0.08);
      for (let i = 0; i < killCount; i++) {
        const victim = enemies[Math.floor(Math.random() * enemies.length)];
        if (victim && !victim.dead) victim.dead = true;
      }
    }
  }

  isDone() {
    return this.phase === 'done';
  }
}

function formatNum(n) {
  if (n >= 1e6) return (n / 1e6).toFixed(1) + 'M';
  if (n >= 1e3) return (n / 1e3).toFixed(1) + 'K';
  return Math.floor(n).toString();
}
